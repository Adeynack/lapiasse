...
// 	StartsAt           time.Time        `gorm:"type:date;not null" json:"starts_at,format:'2006-01-02'" validate:"required,datetime=2006-01-02"` // Opening date of the register (eg: for accounts, but not for categories).
...
// 	AnnualInterestRate *float32         `gorm:"type:real" json:"annual_interest_rate,omitempty" validate:"omitempty,gte=0"`                 // In the case the register is being charged interests, its rate per year (eg: credit card).
// 	CreditLimit        *int64           `gorm:"type:bigint" json:"credit_limit,omitempty" validate:"omitempty,gte=0"`                       // In the case the register has a credit limit (eg: credit card, credit margin).
